...
Analyzer::Analyzer(std::vector<std::string> infiles, std::string outfile, bool setCR, std::string configFolder, std::string year) : goodParts(getArray()), genName_regex(".*([A-Z][^[:space:]]+)"){
  ...
  // New line: 19/06/2024
  // This part of the code therefore reads in the ROOT file with the weights.  The "Read_glgh6_weight" is the flag in Run_info.in
  // that we created that needs to be set to "true" in order to perform this part of the code.
  if(distats["Run"].bfind("Read_glgh6_weight"))
    //std::cout << "Line 175 /t " << std::endl;
    {
    // Here's where the ROOT file is read in.  Notice at first to test the code, we created a dummy ROOT file "dummy_weight.root"
    // with the histogram "h_dummy" and put it in the Pileup folder.
      TFile *wgtfile = new TFile("Pileup/dummy_weight.root","READ");
      for (const std::pair<const std::string, std::string> & n : distats["Run"].smap)
      // The "h_dummy" histogram gets assigned a name of "ZpWeightHist" in Run_info.in.
      if(n.first == "ZpWeightHist")
      {
         // The "hist_Zp_width" is a 1-D histogram defined in the Analyzer.h file. This is where we get the weight and assign it to
         // this histogram name.
         //std::cout << "Line 186 /t " << std::endl;
         hist_Zp_width = (TH1F*) wgtfile->Get((TString)n.second);
         break;
      }
      // This was just for testing purposes.  If you uncomment this here, make sure to uncomment also below the line beginning with
      // "std::cout<<"test! Z' mass "<<_Gen->genmass[idx]<<" . . ."  Note that "genmass" is defined in the file Particle.h.
      //hist_Zp_width = (TH1F*)wgtfile ->Get((distats["Run"].smap.at("ZpWeightHist").cend);
      //std::cout << distats["Run"].smap.at("ZpWeightHist").cend <<std::endl;
    }
...
// New line: 19/06/2024
float Analyzer::ZpWidthReweight()
  {
     //std::cout << "Line 586 /t " << std::endl;
     //std::cout << "hist_Zp_width : " << hist_Zp_width  << std::endl;
     //std::cout << "_Gen->Size() : " << _Gen->size()  << std::endl;

     // If we don't have the hist_Zp_width histogram, throw this error.
     if(!hist_Zp_width) {std::cout<<"Z' reweight function called without loading histogram"<<std::endl; return 1.;}
     // std::cout << "---------------------------" << std::endl;
     // Loop over all generator level particles to look for the leptons that come from a Z.
     for(size_t idx = 0; idx < _Gen->size(); idx++) {
	//std::cout << "For loop : " << idx << std::endl;
       // Get the particle PDG ID and if it is not the Z', then skip.
       if( abs(_Gen->pdg_id[idx]) != 32) continue;

       //std::cout<<"test! Z' mass "<<_Gen->genmass[idx]<<" with test weight "<<hist_Zp_width->GetBinContent(hist_Zp_width->FindBin(_Gen->genmass[idx]))<<std::endl;

       // This is where we find the correct value for the weight and return it.  Note that "genmass" is defined in the file
       // Particle.h.
       //std::cout <<  "Weight: "  << hist_Zp_width->GetBinContent(hist_Zp_width->FindBin(4999)) << "\t\t  M(Z') " << 4999 << std::endl;
       return hist_Zp_width->GetBinContent(hist_Zp_width->FindBin(_Gen->genmass[idx]));
     }
     std::cout<<"No Z' found! Reweight defaults to 1."<<std::endl;
     return 1.;
  }
...
//std::cout<<"PT: "<<" Eta: "<<" tesSF: "<<" tfakerateesSF: "<<std::endl;
//std::cout<<tauRecoP4.Pt()<<" "<<tauRecoP4.Eta()<<" "<<tes_SF<<" "<<tfes_SF<<std::endl;
...
void Analyzer::fill_histogram(std::string year) {
  ...
  if(!isData){
    ...
    myfile << "Negative values \n: "<<di<<std::endl;
    myfile.close();
	     // std::cout<<"Imprimimos di: "<<di<<std::endl;
		    //std::cout<<"PU weight: "<< pu_weight << " Gen Weight: "<< gen_weight <<" Weight wgt:  "<< wgt<<std::endl;
	}*/
    //std::cout<<"di fuera: "<<di <<std::endl;
    //add weight here
    ...
    // New line: 19/06/2024
    //std::cout <<  ZpWidthReweight()  << std::endl;
    if(distats["Run"].bfind("Read_glgh6_weight")) wgt *= ZpWidthReweight();
...
void Analyzer::fill_Folder(std::string group, const int max, Histogramer &ihisto, bool issyst) {
  ...
  if(group == "FillRun" && (&ihisto==&histo)) {
	//std::cout<<"crbins:"<<  crbins<<std::endl;  
  if(crbins != 1) {
      for(int i = 0; i < crbins; i++) {
        std::cout << "nominal, crbins !=1" << std::endl;
        std::cout<<" what is this i: "<<i <<"what is group"<< group<< std::endl;
	ihisto.addVal(false, group, i, "Events", 1);
      ...
      if(isSignalMC){ // For signal samples (unskimmed)
        ...
        // Bin 1 contains the number of events analyzed in a given sample.I
        //dbs+=1;
        //std::cout<<"This is the counter: " << dbs<<std::endl;
	 //   	ihisto.addVal(false, group,ihisto.get_maxfolder(), "Events", 1);
	ihisto.addVal(false, group,ihisto.get_maxfolder(), "Events", (gen_weight > 0) ? 1.0 : -1.0);
	//std::cout<<"THIS is the Group: "<<group<<" THis his the get max folder "<< ihisto.get_maxfolder() <<std::endl;
        if(distats["Run"].bfind("ApplyGenWeight")) {
      ...
      ihisto.addVal(nTruePU, group, ihisto.get_maxfolder(), "PUWeight", 1);
    	//std::cout<<"Peso wgt: "<<wgt<<" nTruePU: "<< nTruePU << std::endl;
	}
    ...
    //std::cout<<"To check if the numbers are the same: "<<"Peso wgt: "<<wgt<<" nTruePU: "<< nTruePU << std::endl;
    // In all cases: Bin 2 contains the weighted number of events passing a certain selection.
    histAddVal(true, "Events");
    histAddVal(bestVertices, "NVertices");
    histAddVal(totalVertices, "NTotalVertices");
    ...
    histAddVal(wgt,"gtWeight");
    histAddVal(pu_weight,"PilesW");
    ...
    for(auto it: *active_part->at(CUTS::eGElec)){
       goodGenLeptons.push_back(it);
      //std::cout<<" estamos acá it: "<< active_part->at(CUTS::eGElec)->size() <<std::endl;
       histAddVal(_Gen->energy(it), "ElectronEnergy");
       histAddVal(_Gen->pt(it), "ElectronPt");
       //histAddVal(_Gen->pt(it)+0.5,"ElectronesPt");
    ...
    for(auto it : *active_part->at(ePos)) {
      ...
      histAddVal(part->p4(it).Pt(),"Pts");

////////////////////////////////////////////////////////////////////////////////

       
	histAddVal2(_MET->pt(), part->p4(it).Pt() , "PtMetTau");
...
//std::cout << active_part->at(ePos)->size() << "  " <<active_part->at(ePos)->size() <<std::endl;
...
//////////////////////////// GrÃfica Tau1-Pt Vs MET ////////////////////////////////

        histAddVal2(part2.Pt(), _MET->pt() , "PtTauMet");
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	std::vector<uint> genleptonindices;
	int genpart_id = 0, genmotherpart_idx = 0, genmotherpart_id = 0;
	int gentau_mother_idx = 0, gentau_mother_id = 0;

	// Variables to store the indices of the taus and the electron
	int tau_hadronic_idx = -1;  // Index of the hadronic tau
	int tau_mother_electron_idx = -1;  // Index of the tau that is the mother of the electron
	int electron_idx = -1;  // Index of the electron


	// Loop over all generated particles
	for (size_t idx = 0; idx < _Gen->size(); idx++) {
		// Get the PDG ID of the current particle
		genpart_id = abs(_Gen->pdg_id[idx]);


		// Get the index and PDG ID of the mother particle
		genmotherpart_idx = _Gen->genPartIdxMother[idx];
		genmotherpart_id = abs(_Gen->pdg_id[genmotherpart_idx]);
	

		// Identify hadronic taus
		if (genpart_id == 15 && _Gen->status[idx] == 2) {  // Unstable tau
			// Check if the tau decays into hadronic particles
			bool is_hadronic = false;
			for (size_t jdx = 0; jdx < _Gen->size(); jdx++) {
				if (_Gen->genPartIdxMother[jdx] == static_cast<int>(idx)) {
					 int daughter_id = abs(_Gen->pdg_id[jdx]);
			                 if (daughter_id == 211 || daughter_id == 321) {  // Pions or kaons
						is_hadronic = true;
						break;
					}
				}
			...
			// If it's a hadronic tau, save its index
			if (is_hadronic && genmotherpart_id == 32) {  // Mother is Z'
				tau_hadronic_idx = idx;
			}
		}


		// Identify electrons that come from a tau
		if (genpart_id == 11 && _Gen->status[idx] == 1) {  // Stable electron
			if (genmotherpart_id == 15) {  // Mother is a tau
				// Save the index of the electron and its tau mother
				electron_idx = idx;
				tau_mother_electron_idx = genmotherpart_idx;

				// Verify that the tau mother comes from a Z'
				gentau_mother_idx = _Gen->genPartIdxMother[tau_mother_electron_idx];
				gentau_mother_id = abs(_Gen->pdg_id[gentau_mother_idx]);
				
				if (gentau_mother_id != 32) {  // If it doesn't come from a Z', discard
					electron_idx = -1;
					tau_mother_electron_idx = -1;
				}
			}
		}
	...
	// Verify that both taus and the electron have been found
	if (tau_hadronic_idx != -1 && tau_mother_electron_idx != -1 && electron_idx != -1) {
		// Get the 4-vectors of the taus and the electron
		TLorentzVector tau_hadronic_p4 = _Gen->p4(tau_hadronic_idx);
		TLorentzVector tau_mother_electron_p4 = _Gen->p4(tau_mother_electron_idx);
		TLorentzVector electron_p4 = _Gen->p4(electron_idx);

		// Calculate the invariant mass of the system (hadronic tau + tau that decays into electron)
		TLorentzVector Zprime_candidate = tau_hadronic_p4 + tau_mother_electron_p4;
		double Zprime_mass_genlevel = Zprime_candidate.M();

		// Fill 2D histogram
                histAddVal2(diMass1, Zprime_mass_genlevel , "mrec_mgen_2D"); // diMass1: Observable in the analysis
                histAddVal(diMass1, "m_rec_test");
                histAddVal(Zprime_mass_genlevel, "mrec_test_v2")


		// Print results (optional)
		//std::cout << "Hadronic tau found at index: " << tau_hadronic_idx << std::endl;
		//std::cout << "Tau mother of the electron found at index: " << tau_mother_electron_idx << std::endl;
		//std::cout << "Electron found at index: " << electron_idx << std::endl;
		//std::cout << "Invariant mass of the Z' candidate: " << Zprime_mass_genlevel << " GeV" << std::endl;		
		 	
	} else {
	//	std::cout << "Required particles were not found." << std::endl;
	}
...
// Verify that both taus, the W, and the electron have been found
if (tau_hadronic_idx != -1 && tau_leptonic_idx != -1 && W_mother_idx != -1 && electron_idx != -1) {
	// Get the 4-vectors of the taus and the electron
	TLorentzVector tau_hadronic_p4 = _Gen->p4(tau_hadronic_idx);
	TLorentzVector tau_leptonic_p4 = _Gen->p4(tau_leptonic_idx);
	TLorentzVector electron_p4 = _Gen->p4(electron_idx);

	// Calculate the invariant mass of the system (hadronic tau + leptonic tau)
	TLorentzVector Zprime_candidate = tau_hadronic_p4 + tau_leptonic_p4;
	double Zprime_mass = Zprime_candidate.M();


	// Print results (optional)
	std::cout << "Hadronic tau found at index: " << tau_hadronic_idx << std::endl;
	std::cout << "Leptonic tau found at index: " << tau_leptonic_idx << std::endl;
	std::cout << "W boson found at index: " << W_mother_idx << std::endl;
	std::cout << "Electron found at index: " << electron_idx << std::endl;
	std::cout << "Invariant mass of the Z' candidate: " << Zprime_mass << " GeV" << std::endl;


	// Fill histograms (if needed)
   	histAddVal(Zprime_mass, "mrec_test_v2");
...
//      double totalmom = sqrt( pow( part1.Pt()*cos(part1.Phi()) + part2.Pt()*cos(part2.Phi()) + _MET->pt()*cos(_MET->phi()),2)
//		
//		+pow( part1.Pt()*sin(part1.Phi()) + part2.Pt()*sin(part2.Phi()) + _MET->pt()*sin(_MET->phi()),2) );
      //double mumupt   = sqrt( pow( part1.Pt()*cos(part1.Phi()) + part2.Pt()*cos(part2.Phi()) ,2)
        //                     +pow( part1.Pt()*sin(part1.Phi()) + part2.Pt()*sin(part2.Phi()) ,2) ) ;
      //histAddVal((part1+part2).Pt(), "TotalMomentumVec");
     // histAddVal( part1.Pt()+part2.Pt()+_MET->pt() , "TotalMomentumEsc");
//std::cout <<active_part->at(CUTS::eRJet1)->size()<< "  "<<_Jet->size()<<"  "<<_GenJet->size()<<"  "<<(part1+part2).Pt()<<"  "<<totalmom
//	  <<"  "<<mumupt<< std::endl;
      //histAddVal2(active_part->at(CUTS::eRJet1)->size(), (part1+part2).Pt(), "NjetsVsTotalMomentumVec");

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


int njets = active_part->at(CUTS::eRJet1)->size();
double ptmumumet = (part1+part2).Pt();
//histAddVal(ptmumumet, "TotalMomentumVec");
//histAddVal2(njets, ptmumumet, "NjetsVsTotalMomentumVec");
//TFile *sffile  = new TFile("Pileup/2dnjetsvspt2016ee.root","READ");
//TH2F *h_sfdy  =(TH2F*)sffile ->Get("NjetsVsTotalMomentumVec");
//sffile ->Close();
//std::cout<<"hola, entra?" <<std::endl;
        //int ptz, nj;
        //if(njets >=6.  ){ nj  =  6;} else{nj  = njets + 1;}
             //if(ptmumumet>=180.){ ptz = 9;} else{ptz = floor(ptmumumet/20.)+1;}
           //  double dysf  = h_sfdy->GetBinContent(nj,ptz);
        //double edysf = h_sfdy->GetBinError(nj,ptz);
        //double dysfp = dysf + edysf;
        //double dysfm = dysf - edysf;
...
//           " +- "<<edysf<<std::endl;
        ...
        //histAddVal2(diMass1, edysf, "mrecnj1err");
        //histAddVal2(diMass1, dysf, "mrecnj1");
        //histAddVal(diMass1, "mrecnj0");
     
  //      histo.addVal(diMass1, "FillElectron1Tau1", 4000, "mrecnj0p", dysfp );
   //     histo.addVal(diMass1, "FillElectron1Tau1", 4000, "mrecnj0m", dysfm );
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

      double mTlead1 = calculateLeptonMetMt(llep);
      double mTmuon1 = calculateLeptonMetMt(part1);
      double mTtau1 = calculateLeptonMetMt(part2);
      histAddVal(mTlead1, "LeadingLeptonMetMt");
      histAddVal2(mTlead1, part1.Pt(), "LeadingLeptonMetMtVsPart1Pt");
      histAddVal2(mTlead1, part2.Pt(), "LeadingLeptonMetMtVsPart2Pt");
      histAddVal2(mTlead1, mTmuon1 , "LeadingLeptonMetMtVsPart1MetMt");
      histAddVal2(mTlead1, mTtau1 , "LeadingLeptonMetMtVsPart2MetMt");
      histAddVal2(mTmuon1, mTtau1 , "Part1MetMtVsPart2MetMt");

      double cosDphiLeadLepMet = cos(absnormPhi(llep.Phi() - _MET->phi()));
      histAddVal(cosDphiLeadLepMet, "RecoCosDphiPtLeadLepandMet");
      histAddVal2(mTlead1, cosDphiLeadLepMet, "LeadingLeptonMetMtVsRecoCosDphiPtLeadLepandMet");
      ...
      histAddVal(calculateLeptonMetMt(part1), "Part1MetMt");
      histAddVal(calculateLeptonMetMt(part2), "Part2MetMt");
      histAddVal(cos(absnormPhi(part1.Phi() - _MET->phi())), "Part1MetDphi");
      histAddVal(cos(absnormPhi(part2.Phi() - _MET->phi())), "Part2MetDphi");
      histAddVal(cos(absnormPhi(part1.Phi() - _MET->phi())), "MuonMetDeltaPhi");
      histAddVal(cos(absnormPhi(part2.Phi() - _MET->phi())), "TauMetDeltaPhi");
      histAddVal2(cos(absnormPhi(part1.Phi() - _MET->phi())), cos(absnormPhi(part2.Phi() - _MET->phi())) , "Part1MetDphiVsPart2MetDphi");
